"""
Integration of OpenCV, easyocr, and Flask for vehicle plate recognition and management.

This module provides functionalities for detecting, recognizing, and managing license plates 
using computer vision techniques and OCR. It includes routes to control camera streaming, 
enable/disable plate recognition, retrieve the latest recognized plate information, and 
analyze login/logout data for reporting.

Dependencies:
- cv2: OpenCV library for image and video processing tasks.
- easyocr: Library for extracting text from images using OCR.
- re: Regular expressions library for text cleaning and validation.
- Flask components: Response, Blueprint, jsonify for web server functionality.
- sqlalchemy: ORM for database interaction.

Global Variables:
- MIN_PLATE_AREA (int): Minimum area for a detected region to be considered a license plate.
- HAARCASCADE_FILE (str): Path to the Haar Cascade XML file for license plate detection.
- OCR_LANGUAGES (list): List of languages supported by the OCR system.
- latest_plate (str or None): Variable to store the most recently recognized plate text.
- plate_recognition_active (bool): Flag to track plate recognition activity.
- plate_cascade (cv2.CascadeClassifier): Haar Cascade classifier for license plate detection.
- reader (easyocr.Reader): Instance of easyocr.Reader for OCR operations.
- camera (cv2.VideoCapture or None): Variable to hold the camera object for video streaming.
- camera_active (bool): Flag to manage the camera's operational state.

Routes:
- /turn_on: Activates the camera for video streaming.
- /turn_off: Deactivates the camera and releases associated resources.
- /video_feed: Provides a streaming video feed of processed frames.
- /start_recognition: Activates the plate recognition process.
- /stop_recognition: Deactivates the plate recognition process.
- /get_latest_plate: Retrieves the latest recognized plate information.
- /get_chart_data: Retrieves aggregated login/logout data for charting.
...
def get_chart_data():
    """
    Retrieve the latest recognized plate information and perform database operations.

    Returns:
    - str: JSON response indicating approval status and latest recognized plate.
    """
